...
Hd_values_10 = subs(H_d, {'L1on', 'L1off', 'L2', 'RL1', 'RL2', 'C', 'Rc', 'Ro', 'vd', 'N1', 'N2', 'd'}, {L1on_eq L1off_eq L3_eq RL1_eq RL3_eq C_eq Rc_eq 10 vd_eq N1_eq N3_eq d_eq});
Hd_values_100 = subs(H_d, {'L1on', 'L1off', 'L2', 'RL1', 'RL2', 'C', 'Rc', 'Ro', 'vd', 'N1', 'N2', 'd'}, {L1on_eq L1off_eq L3_eq RL1_eq RL3_eq C_eq Rc_eq 100 vd_eq N1_eq N3_eq d_eq});

%%
[N, D] = numden(Hd_values_10)
[N_coeffs, a] = coeffs(N, s)
[D_coeffs, b] = coeffs(D, s)
sys10 = tf(double(N_coeffs), double(D_coeffs))
sys = sys10;
step(sys10)

%%
[N, D] = numden(Hd_values_100)
[N_coeffs, a] = coeffs(N, s)
[D_coeffs, b] = coeffs(D, s)
sys100 = tf(double(N_coeffs), double(D_coeffs))
step(sys100)
figure('Name', 'Bode'); bode(sys10, sys100)
grid on
...
fs=100e3
f0=fs/5
fz= f0/100 %f0/tan(boost*pi/360 + pi/4)
fp= f0*100 %tan(boost*pi/360 + pi/4)*f0

loss_f0 = abs(evalfr(Vocomp_Vea,1j*2*pi*f0))
Gea = 1/loss_f0
%%
R2_eq = 5e3*Gea %100e3
R1_eq = 5e3%R2_eq/Gea
C1_eq =  470e-9%1/(2*pi*R2_eq*fz)
C2_eq =  10e-12%1/(2*pi*R2_eq*fp)
